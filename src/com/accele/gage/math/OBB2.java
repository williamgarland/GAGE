...
public class OBB2 extends BoundingBox {

	private float angle;
	
	public OBB2(Vector3f position, Vector3f size, float angle) {
		super(position, size);
		this.angle = angle;
	}
	...
	private Vector3f[] getVertices() {
		return new Vector3f[] {
				new Vector3f(position).add(new Vector3f(-size.x / 2, size.y / 2, 0)),
				new Vector3f(position).add(new Vector3f(size.x / 2, size.y / 2, 0)),
				new Vector3f(position).add(new Vector3f(size.x / 2, -size.y / 2, 0)),
				new Vector3f(position).add(new Vector3f(-size.x / 2, -size.y / 2, 0))
		};
